...
class SSTrainer(BaseTrainer):
  '''
  Source Separation Trainer class
  '''
  ...
  def __init__(
    ...
    self.train_metrics = MetricTracker('loss', 'clf_accuracy', 'grad norm', *[m.name for m in self.metrics], writer=self.writer)
    self.evaluation_metrics = MetricTracker('loss', 'clf_accuracy', *[m.name for m in self.metrics], writer=self.writer)
    self.sisdr = SISDR()

  @staticmethod
  def move_batch_to_device(batch, device: torch.device):
    for tensor_for_gpu in ['mixed', 'target', 'ref', 'target_sp_id']:
  ...
  def _train_epoch(self, epoch):
    ...
    for batch_idx, batch in enumerate(
      ...
      if batch_idx % self.log_step == 0:
        ...
        self._log_spectrogram('target spectrogram', batch['target'])
        self._log_spectrogram('reference spectrogram', batch['ref'])
  ...
  def process_batch(self, batch, is_train: bool, metrics: MetricTracker):
    ...
    outputs = self.model(batch['mixed'], batch['ref'])
